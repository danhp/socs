...
class Model:
    ...
    def __init__(self):
        self.DLId = None
        self.view = None # used for view-dependent model manipulation
...
class View:
    ...
    def set_child(self, child):
        self.child = child
        # set_child may be called after self.init. Therefore the following
        # ensures that the child's init gets called
        if(self.bViewInit is True and hasattr(self.child, 'init')):
            self.child.init()

        if(hasattr(self.child, 'set_view')):
            self.child.set_view(self)
    ...
    def save_screenshot(self, filename = 'screenshot'):
        ...
        im_depth = im_depth.reshape((self.height, self.width))
    ...
    def keyboard(self, key, x, y):
        if hasattr(self.child, 'keyboard'):
            self.child.keyboard(key, x, y)
